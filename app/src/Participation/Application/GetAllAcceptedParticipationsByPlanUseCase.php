...
/**
 * Caso de uso para obtener todas las participaciones aceptadas de un plan.
 */
class GetAllAcceptedParticipationsByPlanUseCase implements IUseCase {
    /**
    ...
    /**
     * Obtiene todas las participaciones aceptadas de un plan.
     *
     * @param int $planId El ID del plan.
     * @return array Un array de objetos Participation que representan las participaciones aceptadas.
     * @throws \Exception Si ocurre algún error durante la operación.
     */
    public function __invoke(int $planId): array {
        try {
            $data = $this->repository->fetchParticipationsByPlan($planId, ParticipationStatus::ACCEPTED);
